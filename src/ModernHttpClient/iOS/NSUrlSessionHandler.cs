		...
		class DataTaskDelegate : NSUrlSessionDataDelegate
		{
			...
			public override void DidReceiveChallenge(NSUrlSession session, NSUrlSessionTask task, NSUrlAuthenticationChallenge challenge, Action<NSUrlSessionAuthChallengeDisposition, NSUrlCredential> completionHandler)
			{
				if (challenge.ProtectionSpace.AuthenticationMethod == NSUrlProtectionSpace.AuthenticationMethodNTLM)
				{
					...
					if (This.Credentials is NetworkCredential networkCredential)
					{
						credentialsToUse = networkCredential;
					}
					else
					{
				...
				if (serverCertChain == null || serverCertChain.Count == 0)
				{
					...
					sslErrorVerify(task, null, chain, errors, completionHandler, challenge);
					return;
				}
				...
				{
					errors = SslPolicyErrors.RemoteCertificateChainErrors;
					sslErrorVerify(task, root, chain, errors, completionHandler, challenge);
					return;
				}
